        ...
        pub fn parse_imm(&self) -> Option<u32> {
            match (self.funct3, self.opcode) {
                ...
                (0b010, RiscvCompressedOpcode::C2) =>
                    return Some(((self.imm_part1 as u32) & 0b11) << 6 | (self.imm_part2 as u32) << 5 | ((self.imm_part1 as u32) & 0b11100)),
                (0b011, RiscvCompressedOpcode::C2) => 
                    return Some(((self.imm_part1 as u32) & 0b111) << 6 | (self.imm_part2 as u32) << 5 | ((self.imm_part1 as u32) & 0b11000)),
                
                (0b010, RiscvCompressedOpcode::C1) | (0b000, RiscvCompressedOpcode::C1) | (0b001, RiscvCompressedOpcode::C1) =>
                    return Some( sign_extend::<u8, u32>((self.imm_part2 as u8) << 7 | (self.imm_part2 as u8) << 6  | (self.imm_part2 as u8) << 5 | (self.imm_part1 as u8)) ),
                (0b011, RiscvCompressedOpcode::C1) => 
                    if self.rd_rs1 != 2 {
                        return Some( sign_extend::<u8, u32>((self.imm_part2 as u8) << 7 | (self.imm_part2 as u8) << 6  | (self.imm_part2 as u8) << 5 | (self.imm_part1 as u8)) << 12 );
                    }else {
                        let non_sign_extended_imm = ((self.imm_part2 as u32)&1) << 9 | (((self.imm_part1 as u32) >> 1)&0b11) << 7 | (((self.imm_part1 as u32) >> 3)&1) << 6 | ((self.imm_part1 as u32) & 0b1) << 5 | ((self.imm_part1 as u32) & 0b10000);
        ...
        pub fn parse_imm(&self) -> Option<u32> {
            match (self.funct3, self.opcode) {
                (0b000, RiscvCompressedOpcode::C0) => {
                    return Some( (((self.imm as u32) >> 2)&0b1111) << 6 | (((self.imm as u32) >> 6)&0b11) << 4 | (((self.imm as u32) >> 0)&1) << 3 | (((self.imm as u32) >> 1)&1) << 2)
                },
        ...
        pub fn parse_imm(&self) -> Option<u32> {
            match (self.funct3, self.opcode){
                (0b110, RiscvCompressedOpcode::C1) | (0b111, RiscvCompressedOpcode::C1) => {
                    ...
                    let non_sign_extended_imm = ((imm_part2 >> 2)&1) << 8 | ((imm_part1 >> 3) & 0b11) << 6 | (imm_part1&1) << 5 | (imm_part2 & 0b11) << 3 | (imm_part1 & 0b110);
                ...
                (0b100, RiscvCompressedOpcode::C1) => {
                    if self.parse_funct2() != 0b10 {
                        ...
                        return Some(sign_extend::<u8, u32>( (self.imm_part2 >> 2) << 7 | (self.imm_part2 >> 2) << 6 | (self.imm_part2 >> 2) << 5 | self.imm_part1))
                    }
                }
        ...
        pub fn parse_imm(&self) -> Option<u32> {
            match (self.funct3, self.opcode) {
                (0b101, RiscvCompressedOpcode::C1) => {
                    Some( (self.imm_bit11 as u32) << 11 | (self.imm_bit10 as u32) << 10 | (self.imm_9_8 as u32) << 8 | (self.imm_bit7 as u32) << 7 | (self.imm_bit6 as u32) << 6 | (self.imm_bit5 as u32) << 5 | (self.imm_bit4 as u32) << 4 | (self.imm_3_1 as u32) << 1 )
                }
                _ => return None
            }
        } 
...
where MemType: EmulatorMemory {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("Riscv64Cpu").field("program_counter", &self.program_counter).field("registers", &self.registers).field("halted", &self.halted).finish()
    }
}
...
where MemType: EmulatorMemory {
    ...
    pub fn tick(&mut self) -> Option<()> {
           ...
           match get_compressed_instruction_type(compressed_inst)? {
                RiscvCompressedInstType::CRType => {
                    ...
                    match (inst.funct4, inst.opcode) {
                        (0b1000, RiscvCompressedOpcode::C2) => 
                            if inst.rs2 == 0 { // C.JR
                                instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                    RiscvOpcode::JALR, 0/*x0*/, 0b000, inst.rd_rs1, 0
                                ).pack().ok()?)
                            } else { // C.MV
                                instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                                    opcode: RiscvOpcode::OP, rd: inst.rd_rs1, funct3: 0b000, rs1: inst.rs2, rs2: 0, funct7: 0b0000000
                                }.pack().ok()?)
                            }
                        
                        (0b1001, RiscvCompressedOpcode::C2) => 
                            if inst.rs2 == 0 { // C.JALR
                                if inst.rd_rs1 != 0 { // C.JALR is only valid when rs1̸=x0; the code point with rs1=x0 corresponds to the C.EBREAK instruction. (RISC-V Volume I, section 16.4)
                                    instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                        RiscvOpcode::JALR, 1/*x1*/, 0b000, inst.rd_rs1, 0
                                    ).pack().ok()?)                             
                                } else { // C.EBREAK
                                    instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                        RiscvOpcode::SYSTEM, 0, 0b000, 0, 1
                                    ).pack().ok()?)    
                                }
                            } else { // C.ADD
                                ...
                                instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                                    opcode: RiscvOpcode::OP, rd: inst.rd_rs1, funct3: 0b000, rs1: inst.rd_rs1, rs2: inst.rs2, funct7: 0b0000000
                                }.pack().ok()?)
                            }
                ...
                RiscvCompressedInstType::CIType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        (0b000, RiscvCompressedOpcode::C2) => // C.SLLI
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::OPIMM, inst.rd_rs1, 0b001, inst.rd_rs1, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b010, RiscvCompressedOpcode::C2) => // C.LWSP
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::LOAD, inst.rd_rs1, 0b010, 2/*sp*/, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b011, RiscvCompressedOpcode::C2) => // C.LDSP
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::LOAD, inst.rd_rs1, 0b011, 2/*sp*/, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b010, RiscvCompressedOpcode::C1) => // C.LI
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::OPIMM, inst.rd_rs1, 0b000, 0/*x0*/, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b011, RiscvCompressedOpcode::C1) =>{
                            if inst.rd_rs1 != 2 {  // C.LUI
                                instruction = u32::from_msb_bytes(&RiscvUTypeInstruction::from(
                                RiscvOpcode::LUI, inst.rd_rs1, inst.parse_imm()?
                                ).pack().ok()?)
                            }else {  // C.ADDI16SP
                                instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                    RiscvOpcode::OPIMM, 2/*sp*/, 0b000, 2/*sp*/, inst.parse_imm()?
                                ).pack().ok()?)
                            }
                        } 

                        (0b000, RiscvCompressedOpcode::C1) => // C.ADDI
                            instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                RiscvOpcode::OPIMM, inst.rd_rs1, 0b000, inst.rd_rs1, inst.parse_imm()?
                            ).pack().ok()?),

                        (0b001, RiscvCompressedOpcode::C1) => // C.ADDIW
                            instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                RiscvOpcode::OPIMM32, inst.rd_rs1, 0b000, inst.rd_rs1, inst.parse_imm()?
                            ).pack().ok()?),
                ...
                RiscvCompressedInstType::CSSType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        ...
                        (0b110, RiscvCompressedOpcode::C2) => // C.SWSP
                        instruction = u32::from_msb_bytes(&RiscvSTypeInstruction::from(
                            RiscvOpcode::STORE, 0b010, 2/*sp*/, inst.rs2, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b111, RiscvCompressedOpcode::C2) => // C.SDSP
                        instruction = u32::from_msb_bytes(&RiscvSTypeInstruction::from(
                            RiscvOpcode::STORE, 0b011, 2/*sp*/, inst.rs2, inst.parse_imm()?
                        ).pack().ok()?),
                ...
                RiscvCompressedInstType::CIWType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        (0b000, RiscvCompressedOpcode::C0) =>  // C.ADDI4SPN
                            instruction = u32::from_be_bytes(RiscvITypeInstruction::from(
                                RiscvOpcode::OPIMM, inst.parse_rd(), 0b000, 2/*sp*/, inst.parse_imm()?
                            ).pack().ok()?),
                        _ => ()
                    }
                },
                ...
                RiscvCompressedInstType::CLType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        ...
                        (0b010, RiscvCompressedOpcode::C0) => // C.LW
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::LOAD, inst.parse_rd(), 0b010, inst.parse_rs1(), inst.parse_imm()?
                        ).pack().ok()?),

                        (0b011, RiscvCompressedOpcode::C0) => // C.LD
                        instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                            RiscvOpcode::LOAD, inst.parse_rd(), 0b011, inst.parse_rs1(), inst.parse_imm()?
                        ).pack().ok()?),
                ...
                RiscvCompressedInstType::CSType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        ...
                        (0b110, RiscvCompressedOpcode::C0) => // C.SW
                        instruction = u32::from_msb_bytes(&RiscvSTypeInstruction::from(
                            RiscvOpcode::STORE, 0b010, inst.parse_rs1(), inst.parse_rs2(), inst.parse_imm()?
                        ).pack().ok()?),

                        (0b111, RiscvCompressedOpcode::C0) => // C.SD
                        instruction = u32::from_msb_bytes(&RiscvSTypeInstruction::from(
                            RiscvOpcode::STORE, 0b011, inst.parse_rs1(), inst.parse_rs2(), inst.parse_imm()?
                        ).pack().ok()?),
                ...
                RiscvCompressedInstType::CAType => {
                    ...
                    match (inst.funct6, inst.funct2, inst.opcode) {
                        (0b100011, 0b11, RiscvCompressedOpcode::C1) => // C.AND
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP, rd: inst.parse_rd_rs1(), funct3: 0b111, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0000000
                        }.pack().ok()?),

                        (0b100011, 0b10, RiscvCompressedOpcode::C1) => // C.OR
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP, rd: inst.parse_rd_rs1(), funct3: 0b110, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0000000
                        }.pack().ok()?),

                        (0b100011, 0b01, RiscvCompressedOpcode::C1) => // C.XOR
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP, rd: inst.parse_rd_rs1(), funct3: 0b100, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0000000
                        }.pack().ok()?),

                        (0b100011, 0b00, RiscvCompressedOpcode::C1) => // C.SUB
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP, rd: inst.parse_rd_rs1(), funct3: 0b000, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0100000
                        }.pack().ok()?),

                        (0b100111, 0b01, RiscvCompressedOpcode::C1) => // C.ADDW
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP32, rd: inst.parse_rd_rs1(), funct3: 0b000, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0000000
                        }.pack().ok()?),

                        (0b100111, 0b00, RiscvCompressedOpcode::C1) => // C.SUBW
                        instruction = u32::from_msb_bytes(&RiscvRTypeInstruction{
                            opcode: RiscvOpcode::OP32, rd: inst.parse_rd_rs1(), funct3: 0b000, rs1: inst.parse_rd_rs1(), rs2: inst.parse_rs2(), funct7: 0b0100000
                        }.pack().ok()?),
                        _ => ()
                    }
                },
                ...
                RiscvCompressedInstType::CBType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        (0b110, RiscvCompressedOpcode::C1) => // C.BEQZ
                        instruction = u32::from_msb_bytes(&RiscvBTypeInstruction::from(
                            RiscvOpcode::BRANCH, 0b000, inst.parse_rd_rs1(), 0/*x0*/, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b111, RiscvCompressedOpcode::C1) => // C.BNEZ
                        instruction = u32::from_msb_bytes(&RiscvBTypeInstruction::from(
                            RiscvOpcode::BRANCH, 0b001, inst.parse_rd_rs1(), 0/*x0*/, inst.parse_imm()?
                        ).pack().ok()?),

                        (0b100, RiscvCompressedOpcode::C1) => // C.SRLI/C.SRAI/C.ANDI 
                            match inst.parse_funct2() {
                                0b00 => // C.SRLI
                                instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                    RiscvOpcode::OPIMM, inst.parse_rd_rs1(), 0b101, inst.parse_rd_rs1(), inst.parse_imm()? 
                                ).pack().ok()?),

                                0b01 => // C.SRAI
                                instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                    RiscvOpcode::OPIMM, inst.parse_rd_rs1(), 0b101, inst.parse_rd_rs1(), inst.parse_imm()? | 0b0100000_00000 
                                ).pack().ok()?),

                                0b10 => // C.ANDI
                                instruction = u32::from_msb_bytes(&RiscvITypeInstruction::from(
                                    RiscvOpcode::OPIMM, inst.parse_rd_rs1(), 0b111, inst.parse_rd_rs1(), inst.parse_imm()? 
                                ).pack().ok()?),
                ...
                RiscvCompressedInstType::CJType => {
                    ...
                    match (inst.funct3, inst.opcode) {
                        ...
                        (0b101, RiscvCompressedOpcode::C1) => // C.J
                        instruction = u32::from_msb_bytes(&RiscvJTypeInstruction::from(
                            RiscvOpcode::JAL, 0/*x0*/, inst.parse_imm()?
                        ).pack().ok()?),
                        _ => ()
                    }      
                },
        ...
        match opcode.get_type() {
            ...
            RiscvInstType::IType => self.execute_itype_inst(RiscvITypeInstruction::unpack(&instruction.to_be_bytes()).ok()?, inst_size),
            RiscvInstType::SType => self.execute_stype_inst(RiscvSTypeInstruction::unpack(&instruction.to_be_bytes()).ok()?),
            RiscvInstType::BType => self.execute_btype_inst(RiscvBTypeInstruction::unpack(&instruction.to_be_bytes()).ok()?, inst_size),
            RiscvInstType::UType => self.execute_utype_inst(RiscvUTypeInstruction::unpack(&instruction.to_be_bytes()).ok()?),
            RiscvInstType::JType => self.execute_jtype_inst(RiscvJTypeInstruction::unpack(&instruction.to_be_bytes()).ok()?, inst_size),
        }
    ...
    fn execute_rtype_inst(&mut self, inst: RiscvRTypeInstruction) {
        match (inst.opcode, inst.funct3, inst.funct7) {
            ...
            (RiscvOpcode::OP, 0b001, 0b0000001) => { // MULH
                let res = sign_extend::<u64, u128>(self.read_reg(inst.rs1)).wrapping_mul( sign_extend::<u64, u128>(self.read_reg(inst.rs2)) );
            ...
            (RiscvOpcode::OP32, 0b000, 0b0000001) => { // MULW
                ...
                self.write_reg(inst.rd, sign_extend((self.read_reg(inst.rs1) as u32).wrapping_mul(self.read_reg(inst.rs2) as u32)));
            }
            ...
            (RiscvOpcode::OP32, 0b100, 0b0000001) => { // DIVW
                self.write_reg(inst.rd, sign_extend((self.read_reg(inst.rs1) as i32).wrapping_div(self.read_reg(inst.rs2) as i32) as u32));
            }

            (RiscvOpcode::OP32, 0b101, 0b0000001) => { // DIVUW
                self.write_reg(inst.rd, sign_extend((self.read_reg(inst.rs1) as u32).wrapping_div(self.read_reg(inst.rs2) as u32)));
            }

            (RiscvOpcode::OP32, 0b110, 0b0000001) => { // REMW
                self.write_reg(inst.rd, sign_extend((self.read_reg(inst.rs1) as i32).wrapping_rem(self.read_reg(inst.rs2) as i32) as u32));
            }

            (RiscvOpcode::OP32, 0b111, 0b0000001) => { // REMUW
                self.write_reg(inst.rd, sign_extend((self.read_reg(inst.rs1) as u32).wrapping_rem(self.read_reg(inst.rs2) as u32)));
            }
    ...
    fn execute_itype_inst(&mut self, inst: RiscvITypeInstruction, inst_size: u64) {
        match (inst.opcode, inst.funct3) {
            ...
            (RiscvOpcode::JALR, 0b000) => { // JALR
                ...
                //     self.halted = true;
                // }
                self.program_counter = new_program_counter.wrapping_sub(inst_size); // Subtract inst_size to counteract the pc increment in the tick function
            },
        ...
        match (inst.opcode, inst.funct3) {
            ...
            (RiscvOpcode::BRANCH, 0b000) => { // BEQ
                if self.read_reg(inst.rs1) == self.read_reg(inst.rs2) {
                    self.program_counter = self.program_counter.wrapping_add(sign_extend_to_u64(inst.parse_imm())).wrapping_sub(inst_size); // Subtract inst_size to counteract the pc increment in the tick function
                }
            },
