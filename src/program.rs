...
use crate::{emulator::Riscv64Cpu, virtmem::LittleEndianVirtualMemory, elf::{ElfFile, elf_program_header, elf_header}, syscall};
...
impl Program {
    pub fn from_elf(elf_data: &[u8]) -> Option<Program> {
        ...
        if elf.header.instruction_set != elf_header::InstructionSet::RiscV { return None; }
        if elf.header.elf_type != elf_header::ElfType::EXECUTABLE { return None; }
        ...
        for header in elf.program_headers {
            if header.segment_type == EnumCatchAll::from(elf_program_header::ProgramHeaderType::Load) {
                let mut segment_data = elf_data[header.segment_file_offset as usize..(header.segment_file_offset+header.segment_file_size) as usize].to_owned();
