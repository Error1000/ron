use crate::{Mutex, primitives::{LazyInitialised, MutexGuard}, program::{Program, ProgramState, WaitInformation, self}, UART};
use alloc::vec::Vec;
    ...
    for i in 0..list.len() {
        ...
        match list[i].as_ref().unwrap().data.state {
            ...
            ProgramState::WAITING_FOR_CHILD_PROCESS(cpid) => {
                ...
                let mut pid_that_we_are_waiting_for_is_invalid = false;
                
                if let Some(cpid) = cpid { // We are waiting for a specific process
                    let child = list.get_mut(cpid);
                    if let Some(Some(child)) = child {
                        ...
                        pid_that_we_are_waiting_for_is_invalid = true;
                    }
                } else { // We are waiting for any child process
                    // Check to see if there are children of us with this id
                    for p in list.iter_mut().filter_map(|val|val.as_mut()) {
                        // Search for procceses that are children waiting, that have a pid of cpid and their parent is us
                        if let ProgramState::TERMINATED_CHILD_WAITING_FOR_PARENT_ACKNOWLEDGEMENT(exit_code) = p.data.state {
                            if p.data.parent_pid.unwrap() == our_pid {
                ...
                }else if pid_that_we_are_waiting_for_is_invalid {
            ...
            ProgramState::TERMINATED_CHILD_WAITING_FOR_PARENT_ACKNOWLEDGEMENT(_) => {
                    // There's nothing we can do
                    // We just wait for the parent to find us and switch our state
            }

            ProgramState::TERMINATED_WAITING_TO_BE_DEALLOCATED(exit_code) => {
                use core::fmt::Write;
                writeln!(UART.lock(), "exit({}) called!", exit_code).unwrap();
