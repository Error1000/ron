  ...
  pub fn from(device_file: Rc<RefCell<dyn IFile>>, partition_number: MBRPartitionNumber) -> Option<Self>{
    ...
    if let Some(part_data) = device_file.borrow().read(part_data_offset, 16){
      ...
      Some(Self{
        ...
        partition_offset: u32::from_le_bytes([part_data[8], part_data[9], part_data[10], part_data[11]]) as u64 * ata::SECTOR_SIZE_IN_BYTES as u64,
        partition_size: u32::from_le_bytes([part_data[12], part_data[13], part_data[14], part_data[15]]) as u64 * ata::SECTOR_SIZE_IN_BYTES as u64,
...
impl IFile for MBRPartitionFile{
    fn read(&self, offset: u64, len: usize) -> Option<Vec<u8>> {
      if offset+len as u64 > self.partition_size { return None; }
      (*self.device).borrow().read(offset+self.partition_offset, len)
    }

    fn write(&mut self, offset: u64, data: &[u8]) -> Option<usize> {
      if offset+data.len() as u64 > self.partition_size { return None; }
