...
impl<STATE: MixedRegisterState> VGAMixedRegister<STATE> {
    unsafe fn typestate_transmute<NewState: MixedRegisterState>(
        self,
...
impl VGAMixedRegister<Blanked> {
    ...
    pub unsafe fn unblank_screen(mut self) -> VGAMixedRegister<Unblanked> {
        self.control.read();
        self.data_and_ind_write
            .write(self.data_and_ind_write.read() | 0x20);
...
impl VGAMixedRegister<Unblanked> {
    pub unsafe fn blank_screen(mut self) -> VGAMixedRegister<Blanked> {
        self.control.read();
        self.data_and_ind_write
            .write(self.data_and_ind_write.read() & (0x20 ^ 0xFF));
...
impl X86Default for Vga<UndefinedVgaMode, Unblanked> {
    unsafe fn x86_default() -> Self {
        Self {
            ...
            graphics_controller_registers: VGARegisterSpace::new(KernPointer::<u8>::from_port(
                0x3CE,
            )),
...
impl<T: VgaMode> Vga<T, Unblanked> {
    pub unsafe fn blank_screen(self) -> Vga<T, Blanked> {
        Vga::<T, Blanked> {
            attribute_controller_registers_mixed: self
                .attribute_controller_registers_mixed
                .blank_screen(),
            misc_register: self.misc_register,
            sequencer_registers: self.sequencer_registers,
...
impl<T: VgaMode + 'static> Vga<T, Blanked> {
    pub unsafe fn dump_all(&mut self, dump: &mut VgaRegisterDump) {
        ...
        for i in 0_u8..=255_u8 {
            dump.attribute_controller_registers_mixed[usize::from(i)] =
                self.attribute_controller_registers_mixed.read(i);
            dump.sequencer_registers[usize::from(i)] = self.sequencer_registers.read(i);
            dump.graphics_controller_registers[usize::from(i)] =
                self.graphics_controller_registers.read(i);
    ...
    unsafe fn load_mode_dump(&mut self, dump: &VgaModeDump) {
        ...
        dump.sequencer_registers
            .iter()
            .for_each(|(index, val)| self.sequencer_registers.write(*index as u8, *val));
        dump.crtc_controller_registers
            .iter()
            .for_each(|(index, val)| self.crtc_controller_registers.write(*index as u8, *val));
        ...
        dump.attribute_controller_registers_mixed
            .iter()
            .for_each(|(index, val)| {
                self.attribute_controller_registers_mixed
                    .write(*index as u8, *val)
            });
    }

    unsafe fn load_font<const SIZE: usize>(
        &mut self,
        font: &[u8; SIZE],
        char_height: u8,
        offset: isize,
        ...
        self.sequencer_registers
            .write(0x04, seq_mode_odd_even_reg_old | 0b100);

        // Switch to flat addressing
        self.graphics_controller_registers
            .write(0x05, graphics_mode_old & !0x10);
        self.graphics_controller_registers
            .write(0x06, misc_old & !0x02);
        ...
        // Restore state
        self.sequencer_registers
            .write(0x04, seq_mode_odd_even_reg_old);
        self.sequencer_registers.write(0x02, seq_mask_reg_old);
        self.graphics_controller_registers
            .write(0x05, graphics_mode_old);
...
impl<STATE: MixedRegisterState> Vga<Text80x25, STATE> {
    ...
    pub unsafe fn write_char(&mut self, x: usize, y: usize, c: u8, color: u8) {
        ...
        vram.offset((y * 80 + x) as isize)
            .write(u16::from_le_bytes([c, color]));
    }
